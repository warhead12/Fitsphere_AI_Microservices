// RabbitMqConfig.java
// Spring AMQP configuration for the Activity Service's RabbitMQ messaging.
// Defines the queue, exchange, binding, and message converter for publishing activity events to the AI Service.
...
public class RabbitMqConfig {
    ...
    @Bean // = "Binds the queue to the exchange with the configured routing key — this is what connects the producer to the consumer"
    public Binding activityBinding(Queue activityQueue, DirectExchange activityExchange) {
        return BindingBuilder.bind(activityQueue).to(activityExchange).with(routingKey);
    }

    @Bean // = "Configures JSON serialization/deserialization for messages so Activity objects are sent as JSON over RabbitMQ"
